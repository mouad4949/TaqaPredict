...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    else if (status === 'connected' && !currentData) setIsLoading(true);
  ...
  if (status === 'connecting') {
    return (
      <div className="flex h-screen bg-gradient-to-br from-green-50 to-teal-50">
        {/* Increased rounding */}
        <div className="text-center m-auto p-8 bg-white rounded-xl shadow-lg">
  ...
  // 2. État d'Erreur ou Déconnexion
  ...
  // 4. État Normal : Connecté et Données Reçues
  if (status === 'connected' && currentData) {
      ...
      <div className="flex h-screen bg-gradient-to-br from-green-50 to-teal-50">
        ...
        <div className="flex-1 p-6 overflow-auto">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Increased rounding for main content block */}
            <div className="mb-6 bg-white rounded-xl shadow-md overflow-hidden">
              {/* Removed rounding from container, added to buttons */}
              <div className="flex border-b">
                <button
                  // Added rounding to tab button
                  className={`px-6 py-3 font-medium rounded-t-lg ${activeTab === "overview" ? "bg-green-600 text-white" : "text-green-600 hover:bg-green-50"}`}
                  onClick={() => setActiveTab("overview")} >
                  Vue d'ensemble
                </button>
                <button
                  // Added rounding to tab button
                  className={`px-6 py-3 font-medium rounded-t-lg ${activeTab === "analytics" ? "bg-green-600 text-white" : "text-green-600 hover:bg-green-50"}`}
                  onClick={() => setActiveTab("analytics")} >
                  Analytique
                </button>
                <button
                  // Added rounding to tab button
                  className={`px-6 py-3 font-medium rounded-t-lg ${activeTab === "reports" ? "bg-green-600 text-white" : "text-green-600 hover:bg-green-50"}`}
              ...
              {activeTab === "overview" && (
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    {/* Increased rounding for card */}
                    <div
                      className={`p-4 rounded-xl shadow-sm ${ // Changed from rounded-lg
                         ...
                         <div>
                           ...
                           <p className="text-sm text-gray-600">
                             {(currentData.payload?.status === "normal" && "Fonctionnement optimal") ||
                              ...
                              (currentData.payload?.status === "moderate" && "Attention requise") ||
                  ...
                  {currentData.payload?.metrics && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                      {/* MetricCard already uses rounded-lg, could change to rounded-xl if desired */}
                  ...
                  {/* Increased rounding for chart container */}
                  <div className="bg-white rounded-xl shadow-md p-4 mb-6"> {/* Changed from rounded-lg */}
                    ...
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                          ...
                          {/* Increased rounding for tooltip */}
                          <Tooltip contentStyle={{ backgroundColor: "rgba(255, 255, 255, 0.9)", border: "1px solid #ccc", borderRadius: "8px", padding: "8px 12px" }}/>
                  ...
                  {/* Increased rounding for log container */}
                  <div className="bg-white rounded-xl shadow-md p-4"> {/* Changed from rounded-lg */}
...
function MetricCard({ title, value, description, color }: MetricCardProps) {
  return (
    // Increased rounding for card
    <div className="rounded-xl shadow-md overflow-hidden"> {/* Changed from rounded-lg */}
    ...
    <div className="p-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2">
           {/* Increased rounding */}
          <div className="bg-white rounded-xl shadow-md p-4 h-full"> {/* Changed from rounded-lg */}
            <h2 className="font-bold text-lg mb-4">Performance Mensuelle</h2>
            <div className="h-80">
               {/* Increased rounding */}
              <div className="flex items-center justify-center h-full bg-gray-50 rounded-xl"> {/* Changed from rounded-lg */}
                <div className="text-center">
                  {/* SVG Icon */}
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto text-green-500 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl shadow-md p-4 h-full"> {/* Changed from rounded-lg */}
            ...
            <div className="space-y-4">
               {/* Increased rounding */}
              <div className="p-3 bg-green-50 rounded-xl"> {/* Changed from rounded-lg */}
                <p className="text-sm text-gray-600">Production Totale</p>
                <p className="text-2xl font-bold">28,456 kWh</p>
              </div>
              <div className="p-3 bg-blue-50 rounded-xl"> {/* Changed from rounded-lg */}
                <p className="text-sm text-gray-600">Efficacité Moyenne</p>
                <p className="text-2xl font-bold">97.8%</p>
              </div>
              <div className="p-3 bg-yellow-50 rounded-xl"> {/* Changed from rounded-lg */}
                <p className="text-sm text-gray-600">Anomalies Détectées</p>
                <p className="text-2xl font-bold">12</p>
              </div>
              <div className="p-3 bg-purple-50 rounded-xl"> {/* Changed from rounded-lg */}
        ...
        <div className="bg-white rounded-xl shadow-md p-4"> {/* Changed from rounded-lg */}
          ...
          <div className="h-64">
            ...
            <div className="flex items-center justify-center h-full bg-gray-50 rounded-xl"> {/* Changed from rounded-lg */}
              <div className="text-center">
                {/* SVG Icon */}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto text-green-500 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {/* ReportCard component itself will handle its rounding */}
        <ReportCard
              ...
              {[
                ...
                <tr key={report.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                     {/* Added rounding to buttons */}
                    <button className="text-green-600 hover:text-green-900 mr-3 px-2 py-1 rounded-md hover:bg-green-50 transition-colors">Télécharger</button>
                    <button className="text-blue-600 hover:text-blue-900 px-2 py-1 rounded-md hover:bg-blue-50 transition-colors">Voir</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white rounded-xl shadow-md p-4"> {/* Changed from rounded-lg */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Type de Rapport</label>
             {/* Increased rounding */}
            <select className="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500"> {/* Changed from rounded-md */}
          ...
          <div className="md:col-span-2">
            ...
            {/* Radio buttons don't typically need rounding change unless customized */}
        ...
        <div className="mt-4 flex justify-end">
           {/* Increased rounding */}
          <button className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"> {/* Changed from rounded-md */}
            Générer le Rapport
          </button>
        </div>
      </div>
...
interface ReportCardProps {
  ...
  description: string
  date: string
  icon: React.ReactNode // Keep type as React.ReactNode
}

function ReportCard({ title, description, date, icon }: ReportCardProps) {
  return (
     // Increased rounding
    <div className="bg-white rounded-xl shadow-md p-4 border-t-4 border-green-500"> {/* Changed from rounded-lg */}
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between items-center mt-3">
            <span className="text-xs text-gray-500">{date}</span>
             {/* Increased rounding slightly */}
            <button className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-md hover:bg-green-200 transition-colors"> {/* Changed from rounded */}
              Télécharger
            </button>
          </div>
        </div>
